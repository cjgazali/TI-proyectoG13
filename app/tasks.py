...
from app.subtasks import get_current_stock, empty_receptions, get_groups_stock, try_manufacture, review_raw_materials, post_to_all, request_raw_materials_c, intermediate_manufacture_c
...
@shared_task
def intermediate():
    print('Estoy en intermediate')
    intermediate_manufacture_c()

@shared_task
def post():
    print('Estoy en post')
    totals = get_current_stock()
    request_raw_materials_c(totals)
